...
exports.handler = function( event, context ) {
	...
	/*
		...
		//'OrderNotes' +
	*/
	var $csvUrl = 'http://affiliate.paidonresults.com/api/transactions?apikey=QPNURHNXUCHYSZHQSWJF' +
		'&Format=CSV' +
	...
	//request('http://affiliate.paidonresults.com/api/overview-report?apikey=QPNURHNXUCHYSZHQSWJF&Format=CSV&FieldSeparator=comma&AffiliateID=40226&Fields=MerchantId,OrderValue,PendingCommission,VoidCommission,ValidCommission&DateFormat=YYYY-MM-DD&Report=date', function (err, res, body) {
	...
	DynamoDB
		...
		.scan(function(err, data) {
			...
			for (var i in data) {
				if (data[i].hasOwnProperty('affiliate_name') && data[i].affiliate_name === 'paidonresults' && data[i].hasOwnProperty('affiliate_id') && data[i].affiliate_id.length )
