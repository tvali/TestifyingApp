...
*/
	...
	DynamoDB
		...
		.query(function( err, data ) {
			...
			console.log("select from revenue where site", event.hash, " where date begins with ", $begin_date )
			DynamoDB
				.table('revenue')
				.where('site').eq( event.hash )
				...
				.query(function( err, data ) {
					...
					console.log($revenue)
					if (err)
						return context.succeed({ success: false, "err":err })
					...
					console.log("Visitors:",$totals.vs);
					console.log("Abandoned carts:",$totals.ab_n);
					console.log("Abandoned rate:",$totals.ab_r);
					console.log("Remarketed:",$totals.popup + $totals.e_s);
					console.log("Recovered carts:",$revenue.r_n);
					console.log("Recovered:",Math.round($revenue.r_a / 100));
					//reportTxt = '{"visitors" :"' + $totals.vs + '", "abdCarts":"'+$totals.ab_n+'" , "abdRate" : "'+$totals.ab_r+'" , "remarketed": "'+($totals.popup + $totals.e_s)+'" ,"recoveredCarts": "'+$revenue.r_n+'","recovered" :"'+ Math.round($revenue.r_a / 100)+'"}'
					reportTxt = '{"visitors":' + $totals.vs + ', "abdCarts":'+$totals.ab_n+' , "abdRate":"'+$totals.ab_r+'", "remarketed":'+($totals.popup + $totals.e_s)+' ,"recoveredCarts":'+$revenue.r_n+',"recovered" :'+ Math.round($revenue.r_a / 100)+'}'
					console.log(reportTxt)
					report = JSON.parse(reportTxt);
					//report = reportTxt;
					console.log(report)
					$tasks--
/*					$body = $body.replace(new RegExp("REPORT_VS".replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'g'),  $totals.vs) ok
					$body = $body.replace(new RegExp("REPORT_AB_N".replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'g'),  $totals.ab_n) ok
					$body = $body.replace(new RegExp("REPORT_AB_RATE".replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'g'),  $totals.ab_r) ok 
...
*/
	...
	setInterval(function() {
		if ($tasks <= 0)
			 context.succeed({ success: true, "report":report })
	},25)
}
