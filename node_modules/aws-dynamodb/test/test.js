...
describe('insert_or_update()', function () {
	it('should should update existing item', function(done) {
		DynamoDB
			.table($tableName)
			.where('hash').eq('hash1')
			.where('range').eq(1)
			.get(function(err,data) {
				DynamoDB
					...
					.insert_or_update({
						...
						delete_me: DynamoDB.del(),
						array: DynamoDB.add( [ 4,'5',6,'7', [], {} ] )
						//array: DynamoDB.
					}, function(err) {
						DynamoDB
							.table($tableName)
...
describe('update', function () {
	...
	it('extended update', function(done) {
		DynamoDB
			.table($tableName)
			.where('hash').eq('hash1')
			.where('range').eq(1)
			.update({
				gsi_range: 'b',
				string: 'string',
				number: DynamoDB.add(5), // is 1 now
					...
					DynamoDB
						...
						.get(function( err, data ) {
							if (err)
								...
								if (data.string !== 'string' ) throw { error: 'unexpected value'}
								if (data.number !== 6 ) throw { error: 'unexpected value'}
								if (data.null !== null ) throw { error: 'unexpected value'}
								// @todo: check all values
								done()
							}
